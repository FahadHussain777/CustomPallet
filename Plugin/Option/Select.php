...
namespace Ec\CustomPallet\Plugin\Option;

use Ec\CustomPallet\Block\Product\View\Options\Type\Select as TypeSelect;
...
class Select
{
    ...
    private function getOptionSwatchHtml($option, $optionValue)
    {
        ...
        if(strpos($optionTitle,'eye') !== false){
            $optionTitle = 'eye_swatch';
        }elseif (strpos($optionTitle,'face') !== false){
            $optionTitle = 'face_swatch';
        }else{
            $optionTitle = 'other_swatch';
        }
        ...
        switch ($type) {
            ...
            case 'image':
            case 'color':
                $html .= '<div class="mageworx-swatch-option image '.$optionTitle;
                $html .= $attributes;
                $html .= ' style="';
    ...
    private function getOptionSwatchHiddenHtml($subject)
    {
        ...
        $select = $subject->getLayout()->createBlock(
            ...
            [
                ...
                'class' => ' mageworx-swatch hidden product-custom-option admin__control-select',
            ]
        );
        if ($_option->getType() == Option::OPTION_TYPE_DROP_DOWN && $_option->getIsSwatch()) {
            ...
            $select->setClass('multiselect admin__control-multiselect product-custom-option');
        }
